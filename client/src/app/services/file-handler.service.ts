...
export class FileService {
 ...
 public upload(fileName: string, fileContent: string): void {
   ...
   this.http.put('/api/files', {name: fileName, content: fileContent})
 ...
 public download(fileName: string): void {
   this.http.get('/api/files/${fileName}', { responseType: 'blob'}).subscribe(res => {
     window.open(window.URL.createObjectURL(res));
   });
 }

 public remove(fileName): void {
   this.http.delete('/api/files/${fileName}').subscribe(() => {
